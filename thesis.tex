...
\TODO{mention lemma a.something about the extension estimate}
\TODO{mention that unless otherwise stated, we mean extension by $\Ext$}
...
\end{align*}
bounds, which are going to be used later to prove the tightness of the $\left\{Z^{:n:}_\gamma, \gamma \in (0, \frac{1}{3})\right\}$ family, and the joint convergence of the (localized) $(Z^{:1:}_\gamma, \ldots, Z^{:n:}_\gamma)$ processes in law to $(Z^{:1:}, \ldots, Z^{:n:})$ with respect to the topology of $\mathcal{D}(\RR_+, \mathcal{C}^{-\nu})^n$.
